...
class UserInfoService {
  constructor() {
    this.performUserInfo = async (code, redirectUri) => {
      logger.log("info", `Get token for user with code ${code} -- ${redirectUri}`);
      ...
      try {
        const response = await axios.post(`${appConfig.nhsRootDomain}/token`, querystring.stringify(formData));
        ...
        const tokenResult = { isValid, tokenResponse: response.data, decodedToken };
